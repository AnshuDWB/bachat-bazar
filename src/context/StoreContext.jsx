...
export function StoreProvider({ children }) {
  const [settings, setSettings] = useState({
    ...
    address:
      "ARAVALI VIHAR, CB-03, NEAR MANSA CHOWK, RTO OFFICE ROAD, BHIWADI, Alwar, Rajasthan, 301019",
    phone: "7073222340",
    ...
    deliveryCharge: 30,
    announcement:
      "📢 Free Home Delivery in Bhiwadi on orders above ₹499! Members save up to 30% extra.",
  });
  ...
  const addCategory = async (categoryData) => {
    try {
      ...
      if (data.success && data.category) {
        ...
        showToast(
          `Category "${data.category.name}" added successfully!`,
          "success",
        );
